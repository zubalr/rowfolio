 ...
 *
 * `it.fails` = demonstrated defect (asserts the correct contract).
 * `it` = verified-good regression lock.
 */
...
describe('adversarial: capture-harness env boundaries', () => {
  it.fails('REPO env var must not alter the shell structure of the harness exec call', () => {
  ...
  it.fails('REPO env path joins must be consistent (no ${REPO}x interpolations)', () => {
...
describe('adversarial: workspace formatting boundaries', () => {
  ...
  it.fails('formatPercentAbs must validate the input before stripping its sign', () => {
  ...
  it.fails('formatPercent must not drop a magnitude on single-fraction-digit inputs (percent-shift defect)', () => {
