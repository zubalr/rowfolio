...
`source/rowfolio.schema.json` is the wire authority for every cross-package payload in Rowfolio. v1.0.0 is frozen after integration-lead (L00) review; until then all changes route through this procedure.

## When a change is needed

1. **Propose, don't merge.** Open a PR or issue against `agent/a01` (owner: A01) describing the exact `$defs` edit and the semantic motivation — e.g. a new `Expression` op, a new metric kind, a renamed field. Contract changes are never piggybacked on feature work.
2. **Lead review.** L00 arbitrates compatibility: additive optional fields may be patch-level; anything that changes required keys, enum/const values, `additionalProperties` strictness or semantic invariants is **breaking**.
...
6. **Coordinated downstream update.** Tasks whose `dependsOn`/`canStartAfter` include A01 (see the task DAG in the handoff) re-run their contract tests against the new version before merge. Schema and implementation land in separate PRs: contract first, consumers second.
7. **Freeze.** Once L00 marks v1 final, further changes go through the same procedure as v2 — never an in-place edit to a frozen version.
...
- [ ] Downstream task IDs that consume the changed type are listed.
