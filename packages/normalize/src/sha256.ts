/**
 * Synchronous SHA-256 over UTF-8 bytes (hex digest).
 *
 * The contracts package exposes an async WebCrypto hasher; engine entry
 * points are synchronous, so each owning package carries this small portable
 * implementation instead of a hashing dependency. Output matches
 * SHA-256 test vectors byte-for-byte (covered in package tests).
 */
