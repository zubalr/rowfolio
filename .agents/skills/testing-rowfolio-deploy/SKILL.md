...
1. Landing `/` — a controllable presentation stage (`data-testid="presentation-stage"`) auto-plays 7 chapters (result → task → check → change → evidence → prepare → deliver, ~92s). Transport pill: `presentation-prev|pause|play|next`, chapter dots `presentation-chapter-{id}`, persistent `presentation-workspace` entry. Any interaction pauses; resume only via `presentation-play`. Held end state shows `presentation-end-workspace`, `presentation-download-pptx`, `presentation-download-xlsx`, `presentation-replay`.
2. Navigate to `#/workspace` (address bar; hash route, no server route) OR use any stage/hand-off CTA — they stash a one-shot `WorkspaceIntent` (`src/landing/pendingUpload.ts`) then navigate. `WorkspaceScreen` mount consumes the intent → `controller.useSample()` → fetch `/sample/index.json` + `/sample/sample_operations.xlsx` → ingest/profile/analyze inside `new Worker('/assets/analysis.worker-*.js', {type:'module'})` → `ready` phase (KpiStrip, FindingList, "Prepare briefing" `export-prepare-btn`).
   - `setWorkspaceIntent({kind:'sample', download:'pptx'|'xlsx'})` (the S7 download buttons) additionally opens the export dialog and auto-downloads the chosen artifact once the pipeline finishes.
   - Below-stage CTAs: `cta-demo` ("Check the data") stashes `{kind:'sample'}` + scrolls to the `#demo` specimen (does NOT navigate); `cta-upload` opens a file picker that stashes `{kind:'upload', file}` + navigates; `cta-guide` stashes `{kind:'guide'}` + scrolls to the specimen.
